...
class AutoWebsiteVisitor:
        ...
        try:
            with BrowserManager(self.settings, self.logger) as browser:
                ...
                for visit_num in range(1, self.settings.visit_count + 1):
                    self.logger.info(f"Starting visit {visit_num}/{self.settings.visit_count}")
                    
                    success = self._perform_visit_with_retry()
                    
                    if success:
                        self.success_count += 1
                        self.logger.info(f"Visit {visit_num} completed successfully")
                    else:
                        self.failure_count += 1
                        self.logger.error(f"Visit {visit_num} failed")
                    
                    self.visit_count += 1
                    
                    # Wait between visits (except for the last one)
                    if visit_num < self.settings.visit_count and self.settings.interval > 0:
                        self.logger.info(f"Waiting {self.settings.interval} seconds before next visit")
            ...
            try:
                if self.browser_manager.visit_website():
                    ...
                    if attempt < self.settings.retry_attempts:
                        self.logger.warning(f"Visit attempt {attempt} failed, retrying in {self.settings.retry_delay} seconds")
                        time.sleep(self.settings.retry_delay)
                    else:
                        self.logger.error(f"All {self.settings.retry_attempts} attempts failed")
                        return False
            except Exception as e:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "success_rate": (self.success_count / self.visit_count * 100) if self.visit_count > 0 else 0
        }