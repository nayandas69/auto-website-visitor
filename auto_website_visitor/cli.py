...
@click.option('--browser', '-b', type=click.Choice(['chrome', 'firefox', 'edge']), help='Browser to use')
...
@click.option('--random-delay', is_flag=True, help='Enable random delays between actions')
...
def main(ctx, **kwargs):
    ...
    config.update_from_args(**{k: v for k, v in kwargs.items() if v is not None})
...
def configure_and_run(config: Config) -> None:
    ...
    # Basic settings
    url = click.prompt("Website URL", default=config.settings.url or "https://example.com")
    config.settings.url = url
    
    count = click.prompt("Number of visits", type=int, default=config.settings.visit_count)
    config.settings.visit_count = count
    
    interval = click.prompt("Interval between visits (seconds)", type=int, default=config.settings.interval)
    config.settings.interval = interval
    
    # Browser settings
    browser = click.prompt("Browser", type=click.Choice(['chrome', 'firefox', 'edge']), default=config.settings.browser)
...
def configure_advanced_options(config: Config) -> None:
    ...
    auto_scroll = click.confirm("Enable auto-scroll?", default=config.settings.auto_scroll)
    config.settings.auto_scroll = auto_scroll
    
    random_delay = click.confirm("Enable random delays?", default=config.settings.random_delay)
    config.settings.random_delay = random_delay
    
    user_agent = click.prompt("Custom user agent (leave empty for default)", default=config.settings.user_agent or "", show_default=False)
    config.settings.user_agent = user_agent if user_agent else None
    
    proxy = click.prompt("Proxy server (leave empty for none)", default=config.settings.proxy or "", show_default=False)
    config.settings.proxy = proxy if proxy else None
    
    retry_attempts = click.prompt("Retry attempts", type=int, default=config.settings.retry_attempts)
    config.settings.retry_attempts = retry_attempts
    
    log_level = click.prompt("Log level", type=click.Choice(['DEBUG', 'INFO', 'WARNING', 'ERROR']), default=config.settings.log_level)
...
def configure_scheduling(config: Config) -> None:
    ...
    schedule_type = click.prompt("Schedule type", type=click.Choice(['interval', 'cron']), default=config.settings.schedule_type)
    config.settings.schedule_type = schedule_type
    
    if schedule_type == "interval":
        click.echo("Examples: 1h, 30m, 45s")
        schedule_value = click.prompt("Interval", default=config.settings.schedule_value)
    else:
        click.echo("Examples: */30 * * * * (every 30 minutes), 0 9 * * 1-5 (weekdays at 9 AM)")
...
def load_config_interactive(config: Config) -> None:
    ...
    config_path = click.prompt("Configuration file path", default=config.get_default_config_path())
...
def save_config_interactive(config: Config) -> None:
    ...
    config_path = click.prompt("Save configuration to", default=config.get_default_config_path())
    format_type = click.prompt("Format", type=click.Choice(['yaml', 'json']), default='yaml')
