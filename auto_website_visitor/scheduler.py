...
class SchedulerManager:
    ...
    def schedule_job(self, job_func: Callable, schedule_type: str, schedule_value: str) -> None:
    ...
    def _schedule_interval(self, job_func: Callable, interval_str: str) -> None:
        ...
        def interval_job():
            while not self.stop_event.is_set():
                try:
                    self.logger.info(f"Executing scheduled job (interval: {interval_str})")
                    job_func()
                    self.logger.info(f"Scheduled job completed, next run in {interval_str}")
